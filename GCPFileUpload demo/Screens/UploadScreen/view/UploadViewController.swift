...
extension UploadViewController: UploadViewDelegate {
    func onProgressChanged(progress: Float) {
        ...
        if progress >= 100 {
            showNotifications(progress: "Upload Completed Successfully")
        }else {
    ...
    func onDataSuccess(downloadUrl: String) {
        ...
        let alertVC = UIAlertController(title: "File Uploaded", message: "file uploaded successfully", preferredStyle: .alert)
    ...
    func onDataError(message: String) {
        print(message)
        let alertVC = UIAlertController(title: "File Uploaded", message: "file upload failed", preferredStyle: .alert)
        alertVC.addAction(UIAlertAction(title: "ok", style: .default, handler: { [weak self] (action) in
            self?.progressBar.progress = 0
            self?.uploadProgressLbl.text = "start upload"
        }))
