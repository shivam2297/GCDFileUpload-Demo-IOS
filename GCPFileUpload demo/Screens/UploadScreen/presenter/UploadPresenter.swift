...
class UploadPresenter {
    ...
    func uploadData(data: Data) {
        ...
        let imageRef = storageRef.child("images/" + randomString() + ".jpeg")
        ...
        uploadTask.observe(.failure) { [weak self] (snapshot) in
            self?.uploadView?.onDataError(message: "Upload failed: \(snapshot.error?.localizedDescription)")
        }
    ...
    func uploadFile(url: URL) {
        ...
        let fileRef = storageRef.child("videos/" + randomString() + ".MOV")
        let metadata = StorageMetadata()
        metadata.contentType = "video/mov"

        let uploadTask = fileRef.putFile(from: url, metadata: metadata)

        addObservers(uploadTask: uploadTask, fileRef: fileRef)
...
//        }
    ...
    func addObservers(uploadTask: StorageUploadTask, fileRef: StorageReference) {
        //observer for progress
        uploadTask.observe(.progress) { [weak self] (snapshot) in
